...
export default async function PlayerStatsPage() {
  ...
  const [{ data: players }, { data: stats, error }, { data: games }] = await Promise.all([
  ...
  const totals: Record<string, { goals: number; assists: number; cleanSheets: number }> = {}
  for (const s of stats ?? []) {
    const t = (totals[s.player_id] ??= { goals: 0, assists: 0, cleanSheets: 0 })
    t.goals += s.goals
    t.assists += s.assists
  ...
  const leaderboard = (players ?? [])
    .map((p) => ({ player: p, ...(totals[p.id] ?? { goals: 0, assists: 0, cleanSheets: 0 }) }))
    .filter((r) => r.goals > 0 || r.assists > 0 || r.cleanSheets > 0)
    .sort((a, b) => b.goals - a.goals || b.assists - a.assists || b.cleanSheets - a.cleanSheets)

  const mine = me ? totals[me.id] ?? { goals: 0, assists: 0, cleanSheets: 0 } : null
          ...
          <div className="mb-6 grid grid-cols-3 gap-3">
            ...
            <div className="card flex flex-col items-center gap-1 p-3.5">
              <div className="font-display text-2xl font-bold leading-none text-white">
                {mine.cleanSheets > 0 ? mine.cleanSheets : mine.goals + mine.assists}
              </div>
              <div className="text-[11px] text-slate-400">
                {mine.cleanSheets > 0 ? 'Clean sheets' : 'Points'}
              </div>
            </div>
          </div>
        </>
            ...
            {myGameStats.map(({ stat, game }) => (
              <div key={stat.game_id} className="flex items-center justify-between px-4 py-2.5 text-sm">
                ...
                <div className="shrink-0 text-xs text-slate-300">
                  {stat.goals > 0 && <span className="mr-2">G {stat.goals}</span>}
